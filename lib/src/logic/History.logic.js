...
HistoryManager.addUpdate = (user, sk, descriptors) => __awaiter(void 0, void 0, void 0, function* () {
    const { displayName } = user, userId = user.sk;
    try {
        const history = new History_model_1.History.Config({ sk }), update = new History_model_1.History.Update(Object.assign({ sk, userId, displayName, historyId: history.sk }, descriptors));
        ...
        ErrorLogging_logic_1.AdminLogManager.logError(error, {
            ...
            from: "History.logic/createHistory",
        });
    }
});
