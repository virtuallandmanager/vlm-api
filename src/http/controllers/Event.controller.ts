...
router.post('/link/giveaways', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { eventId, giveawayId, giveawayLinkIds, eventLinkIds } = req.body
    let giveawayLinks = []
...
router.get('/migrate/all', async (req: Request, res: Response) => {
  try {
    ...
    await giveawayItems.forEach(async (item: Giveaway.Item) => {
      const gItem = new Giveaway.Item({
        ...item,
        imageSrc: `https://peer.decentraland.org/lambdas/collections/contents/urn:decentraland:matic:collections-v2:${item.contractAddress}:${item.itemId}/thumbnail`,
      })
      gItems.push(gItem)
