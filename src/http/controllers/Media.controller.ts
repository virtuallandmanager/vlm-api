...
router.get('/channel/all', authMiddleware, async (req: Request, res: Response) => {
  try {
    const channels = await MediaManager.getCommunityChannels()
...
router.post('/channel/add', authMiddleware, async (req: Request, res: Response) => {
  try {
    const channel = new Media.Channel(req.body.channel)
    await MediaManager.addCommunityChannel(channel)
    const channels = await MediaManager.getCommunityChannels()
    return res.status(200).json({
      channels,
    })
