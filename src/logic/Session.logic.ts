...
const hasConsistentInterval: CallableFunction = (
...
const rateLimitAnalyticsAction: CallableFunction = (
...
export abstract class SessionManager {
  static initAnalyticsSession: CallableFunction = async (
    config: Analytics.Session.Config
  ) => {
    try {
      ...
      AdminLogManager.logError(
        "Failed to initialize analytics session",
        config
      );
      console.log(error);
      return;
  ...
  static startAnalyticsSession: CallableFunction = async (
    config: Analytics.Session.Config
  ) => {
    try {
      const recentSession = await SessionDbManager.getRecentAnalyticsSession(
        config.userId
      );
      if (recentSession) {
      ...
      return (
        recentSession ||
        (await SessionDbManager.start(new Analytics.Session.Config(config)))
      );
    } catch (error: any) {
  ...
  static startBotSession: CallableFunction = async (
    config: Analytics.Session.BotConfig
  ) => {
    try {
      const recentSession = await SessionDbManager.getRecentAnalyticsSession(
        config.userId
  ...
  static refreshSession: CallableFunction = async (
  ...
  static getAnalyticsSession: CallableFunction = async (
    config: Analytics.Session.Config
  ) => {
    try {
      const session = new Analytics.Session.Config(config);
  ...
  static endAnalyticsSession: CallableFunction = async (
    ...
    try {
      if (session && session.pk && session.sk && !session.sessionEnd) {
        ...
        AdminLogManager.logError(
          "Tried to end a session that is already over",
          session.sk
        );
      } else {
  ...
  static storePreSession: CallableFunction = async (
  ...
  static startVLMSession: CallableFunction = async (
    config: User.Session.Config
  ) => {
    try {
  ...
  static getVLMSession: CallableFunction = async (
    config: User.Session.Config
  ) => {
    try {
      if (config.sk) {
        ...
        const activeSessions = await SessionDbManager.activeVLMSessionsByUserId(
          config.userId
        );
        ...
        await activeSessions.forEach(
          async (session: User.Session.Config, i: number) => {
            if (i > 0) {
  ...
  static endVLMSession: CallableFunction = async (
    config: User.Session.Config
  ) => {
    try {
  ...
  static issueUserSessionToken: CallableFunction = (
    session: User.Session.Config
  ) => {
  ...
  static issueAnalyticsSessionToken: CallableFunction = (
    session: Analytics.Session.Config
  ) => {
  ...
  static issueRefreshToken: CallableFunction = (
    session: User.Session.Config
  ) => {
  ...
  static issueSignatureToken: CallableFunction = (
    session: Analytics.Session.Config | User.Session.Config
  ) => {
  ...
  static validateUserSessionToken: CallableFunction = async (
    sessionToken: string
  ) => {
    let decodedSession;
    try {
  ...
  static validateAnalyticsSessionToken: CallableFunction = async (
    sessionToken: string
  ) => {
    let decodedSession;
    try {
  ...
  static validateSignatureToken: CallableFunction = async (config: {
    ...
    let dbSession,
      decodedSession,
      { signatureToken, signature, signatureAccount, signatureMessage } =
        config;
    try {
    ...
    try {
      ...
      if (
        ![initialAddress.toLowerCase(), reportedAddress.toLowerCase()].every(
          (address) => address == actualAddress.toLowerCase()
        )
      ) {
  ...
  static validateSessionRefreshToken: CallableFunction = async (
    refreshToken: string
  ) => {
    ...
    try {
      decodedSession = jwt.verify(
        refreshToken,
        process.env.JWT_REFRESH
      ) as Partial<User.Session.Config>;
    } catch (error) {
      return false;
    }
  ...
  static createSessionPath: CallableFunction = async (
    sessionConfig: Analytics.Session.Config,
    sessionPathConfig?: Analytics.Path
  ) => {
  ...
  static extendPath: CallableFunction = async (
    pathId: string,
    pathSegments: Analytics.PathSegment[]
  ) => {
    const segments = pathSegments.map(
      (segment: Analytics.PathSegment) =>
        new Analytics.PathSegment({ ...segment, pathId })
    );
    return await SessionDbManager.addPathSegments(pathId, segments);
  };

  static addPath: CallableFunction = async (
    userSessionPath: Analytics.Path
  ) => {
    return await SessionDbManager.createPath(userSessionPath);
  };

  static getSessionPath: CallableFunction = async (
    userSessionPath?: Analytics.Path
  ) => {
