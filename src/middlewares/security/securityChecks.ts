...
export async function runChecks(
  ...
  const validOrigin = (TESTS_ENABLED && metadata.realm.serverName === 'LocalPreview')
    ? true
    : checkOrigin(req)
  if (!validOrigin) {
  ...
  const validCatalystPos: boolean = (TESTS_ENABLED && metadata.realm.serverName === 'LocalPreview')    ? true
    : await checkPlayer(
      userAddress,
